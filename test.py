...
def vergleicheTirschenreuthMitCheb():
    filtered_df.drop(filtered_df.index[(filtered_df["district"] != "CZ0411")], axis=0, inplace=True)  # Funktion für Landrkeis Auswahl

    filtered_df.drop(filtered_df.columns.difference(['date_cz', 'district_cz_infected_number']), 1, inplace=True)
    print(filtered_df.to_string())
    df_cleaned = df.dropna()

    filtered_df2['district_de_infected_number'] = filtered_df2['9377']  # Hier Landkreis Schlüssel ändern (Hier Tirschenreuth)
